...
import { LoginTypeDTO } from '../login-type/dto/login-type.dto';
...
export class UserAuthService implements OnModuleInit {
  ...
  async unregister(id: string) {
    ...
    const unregisterResult = await this.userAuthRepository.delete(userAuthId);
  ...
  async attachRole(userAuthId: string, roleId: string) {
    try {
      ...
      const userAuth = await this.userAuthRepository.update(userAuthId, {role});
  ...
  async seekPermission(payload: any, roles: any, permissions: any) {
    ...
    for (const role of roles) {
      if (user.role.name === role && user.role.status) {
        validRole = true;
        break;
      }
    }
    ...
    for (const rp of roleAndPermissions.permissions) {
      for (const permission of permissions) {
        if (rp.action === permission) {
  ...
  async findAuthWithLoginTypeId(loginTypeId: string): Promise<UserAuthDTO[]> {
    const options: FindManyOptions = {
      where: {
        loginType: loginTypeId,
      },
    };
    return await this.userAuthRepository.find(options);
  }
}
